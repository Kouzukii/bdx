...
readFile("real_names.yml", 'utf8', (err, data) => {
  ...
  try {
    ...
    customGlobs = doc.mappings.custom.map(split).map(g => [new Minimatch(g[0], {nocase: true}), g[1]]);
    doc.mappings.armor.forEach(s => {
      const g = split(s);
      armorGlobs.push([new Minimatch(`/character/**/${g[0]}.pac`), g[1]]);
      armorGlobs.push([new Minimatch(`/character/**/${g[0]}_dm.pac`), g[1] + " Damaged"]);
    });
    classGlobs = doc.mappings.class.map(split).map(g => [new Minimatch("/character/model/1_pc/" + g[0], {nocase: true}), g[1]]);
  } catch (e) {
