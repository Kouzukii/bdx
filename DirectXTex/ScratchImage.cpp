...
namespace DirectXTexNet
{
	...
	ScratchImage^ ScratchImage::Resize(Int32 width, Int32 height, bool decompress)
	{
		HRESULT hr;
		DirectX::ScratchImage* newScratchImage;
		auto source = scratchImage_->GetImage(0, 0, 0);
		if (decompress)
		{
			newScratchImage = new DirectX::ScratchImage();
			hr = DirectX::Decompress(*source, DXGI_FORMAT_UNKNOWN, *newScratchImage);
			Marshal::ThrowExceptionForHR(hr);
			source = newScratchImage->GetImage(0, 0, 0);
		}

		auto result = gcnew ScratchImage();
		hr = DirectX::Resize(*source, width, height, DirectX::TEX_FILTER_FANT, *result->scratchImage_);
		Marshal::ThrowExceptionForHR(hr);

		if (decompress)
		{
			const DXGI_FORMAT format = scratchImage_->GetMetadata().format;
			hr = DirectX::Compress(*result->scratchImage_->GetImage(0,0,0), format, DirectX::TEX_COMPRESS_DEFAULT, DirectX::TEX_THRESHOLD_DEFAULT, *newScratchImage);
			Marshal::ThrowExceptionForHR(hr);
			delete result->scratchImage_;
			result->scratchImage_ = newScratchImage;
		}

		return result;
	}

	void ScratchImage::SaveToDDS(String^ path)
	{
		pin_ptr<const wchar_t> filenameCStr = PtrToStringChars(path);

		HRESULT hr = DirectX::SaveToDDSFile(scratchImage_->GetImages(), scratchImage_->GetImageCount(), scratchImage_->GetMetadata(), DirectX::DDS_FLAGS_NONE, filenameCStr);
		Marshal::ThrowExceptionForHR(hr);
	}
}
